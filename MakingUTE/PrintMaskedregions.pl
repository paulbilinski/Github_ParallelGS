...
die "usage: Mask_Blasthits.pl <BLAST RESULT> <TE_DB> <Dashes> <Masked Out> <MaskedPieces>" unless @ARGV==5;
...
#			print "$hashlookup";
#			print "$TE_end_mask\n";
			my $maskedpart = substr ($hashlookup, $TE_start_mask, $TE_length_mask);
			print PART ">$count\n$maskedpart\n";
			$count++;
