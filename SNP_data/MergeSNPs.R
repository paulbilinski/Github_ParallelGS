setwd("~/Documents/Projects/Genome_Size_Analysis/Github_ParallelGS/SNP_data/")
...
#write.csv(all4, file="MergedSNPs2.csv") #this is the checkpoint that produces all SNPs merged into 1

data <- read.csv("MergedSNPs2.csv",header=TRUE)

crap <- colnames(data)
#write.csv(crap,file="SNPsampleIDs.csv") #colum names for the accessions, need to reconcile with the list i have phenotypic data for
...
#write.csv(data3,file="GenosForConvert.csv") # print the 
